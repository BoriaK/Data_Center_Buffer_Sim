...
Queue_Length_Arr = torch.zeros(N_ports, max_Queues, Length * upsample_factor)
...
for k in range(Length):  # for each incoming stream in time t
    ...
    for t in range(upsample_factor):
        Queue_Length_t = torch.zeros(N_ports, max_Queues)
        ...
        for i in range(N_ports):
            for j in range(N_streams[i]):
                if states[i][j] == 'transition':
                    ...
                    # calculate the Qi(t))
                    if t == 0:
                        Queue_Length_t[i, j] = 0  # each queue is empty at the beginning of arrival of data
                        Queue_Length_Arr[
                            i, j, k * upsample_factor + t] = Queue_Length_t[i, j]
                    else:
                        Queue_Length_t[i, j] = Rates_Arr[i, j, t] * 1 / upsample_factor  # Qi(t) = Ri(t) * t
                        Queue_Length_Arr[i, j, k * upsample_factor + t] = Queue_Length_Arr[
                                                                              i, j, k * upsample_factor + t - 1] + \
                                                                          Queue_Length_t[i, j]

                elif states[i][j] == 'steady':
                    Rates_Arr[i, j, t] = 0
                    Queue_Length_t[i, j] = Rates_Arr[i, j, t] * 1 / upsample_factor  # Qi(t) = Ri(t) * t
                    Queue_Length_Arr[i, j, k * upsample_factor + t] = Queue_Length_Arr[
                                                                          i, j, k * upsample_factor + t - 1] + \
                                                                      Queue_Length_t[i, j]
        # calculate the Q(t) and Threshold
        if t == 0:
            Q[k * upsample_factor + t] = 0  # [Packets/sample]
        else:
            Q[k * upsample_factor + t] = Q[k * upsample_factor + t - 1] + Queue_Length_t.sum((0, 1))
            ...
            for j in range(N_streams[i]):
                if j == 0:
                    ...
                    Delta_Arr[i, j, t] = Threshold[0, k * upsample_factor + t] - \
                                         Queue_Length_Arr[i, j, k * upsample_factor + t]
                    if Delta_Arr[i, j, t] < 0:
                        ...
                        Intr = (Threshold[0, k * upsample_factor + t - 2] * Queue_Length_Arr[
                            i, j, k * upsample_factor + t - 1] -
                                Threshold[0, k * upsample_factor + t - 1] * Queue_Length_Arr[
                                    i, j, k * upsample_factor + t - 2]) \
                               / (Threshold[0, k * upsample_factor + t - 2] - Threshold[
                            0, k * upsample_factor + t - 1] +
                                  Queue_Length_Arr[i, j, k * upsample_factor + t - 1] - Queue_Length_Arr[
                                      i, j, k * upsample_factor + t - 2])
                        # Make the correction to Queue length, Threshold, and R(t)
                        Queue_Length_t[i, j] = Intr - Queue_Length_Arr[i, j, k * upsample_factor + t - 1]
                        Queue_Length_Arr[i, j, k * upsample_factor + t] = Intr
                        Threshold[0, k * upsample_factor + t] = Queue_Length_Arr[i, j, k * upsample_factor + t]
                    ...
                    Delta_Arr[i, j, t] = Threshold[1, k * upsample_factor + t] - Queue_Length_Arr[
                    ...
                    if Delta_Arr[i, j, t] < 0:
                        ...
                        Rates_Arr[i, j, t] = 0
                        # calculate the intersection point between High Priority Threshold and a specific Queue length
                        Intr = (Threshold[1, k * upsample_factor + t - 2] * Queue_Length_Arr[
                            i, j, k * upsample_factor + t - 1] -
                                Threshold[1, k * upsample_factor + t - 1] * Queue_Length_Arr[
                                    i, j, k * upsample_factor + t - 2]) \
                        ...
                        Queue_Length_t[i, j] = Intr - Queue_Length_Arr[i, j, k * upsample_factor + t - 1]
                        Queue_Length_Arr[i, j, k * upsample_factor + t] = Intr
                        Threshold[1, k * upsample_factor + t] = Queue_Length_Arr[i, j, k * upsample_factor + t]
                        # re-calculate Q(t) and the other Threshold
                        Q[k * upsample_factor + t] = Q[k * upsample_factor + t - 1] + Queue_Length_t.sum((0, 1))
                        Threshold[0][k * upsample_factor + t] = alpha_high * (
...
for i in range(N_ports):
    for j in range(N_streams[i]):
        ...
        plt.plot(Time, Queue_Length_Arr[i, j, :])
        if j == 0:
