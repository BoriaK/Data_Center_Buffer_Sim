...
for k in range(Length):  # for each incoming stream in time t
    ...
    Queue_Length_Arr = torch.zeros(N_ports, max_Queues, upsample_factor)  # num of packets in each queue
    ...
    for t in range(upsample_factor):
                    ...
                    if j == 0:
                        ...
                        Intr = (Threshold[1, k * upsample_factor + t - 2] * Queue_Length_Arr[
                            ...
                            1, k * upsample_factor + t - 1] +
                                  Queue_Length_Arr[i, j, k * upsample_factor + t - 1] - Queue_Length_Arr[
                                      i, j, k * upsample_factor + t - 2])
                    x1 = Queue_Length_Arr[i, j, k * upsample_factor + t - 1] - Intr
        ...
        for i in range(N_ports):
            for j in range(N_streams[i]):
                if t == 0:
                    Queue_Length_Arr[
                        i, j, k * upsample_factor + t] = 0  # each queue is empty at the beginning of arrival of data
                else:
                    Queue_Length_Arr[i, j, k * upsample_factor + t] = Queue_Length_Arr[
                                                                          i, j, k * upsample_factor + t - 1] + \
                                                                      Rates_Arr[
                                                                          i, j, k * upsample_factor + t] * 1 / upsample_factor  # Ri*t = Qi length
