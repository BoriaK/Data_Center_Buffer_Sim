...
for i in range(N_ports):
    for j in range(N_streams[i]):
        ...
        # Traffic[i, j, :] = torch.tensor([2]) * R_max[i, j]  # Special case: Q(t)>B  single
        Traffic[i, j, :] = torch.tensor([2, 0, 1]) * R_max[i, j]  # Special case: Q(t)>B  multi queue
    ...
    for t in range(upsample_factor):
        ...
        for i in range(N_ports):
            for j in range(N_streams[i]):
                if k == 0 and t == 0:
                ...
                elif Delta_Arr[i, j, k * upsample_factor + t - 1] > 0 or (
                        k > 0 and Traffic[i, j, k] != Traffic[i, j, k - 1]):
                    # if in transient state
                    Rates_in_Arr[i, j, k * upsample_factor + t] = Traffic[i, j, k] - C  # max TpT for incoming packets
                    # calculate the Qi(t))
                ...
                elif Delta_Arr[i, 0, k * upsample_factor + t - 1] == 0 and Delta_Arr[i, 1, k * upsample_factor + t - 1] == 0:
                    # definate steady state:
                    ...
                    Queue_i_Length_Arr[i, j, k * upsample_factor + t] = torch.max(Queue_i_Length_Arr[
                                                                                  ...
                                                                                  Queue_Length_dt[i, j],
                                                                                  torch.tensor(0))
                elif Delta_Arr[i, j, k * upsample_factor + t - 1] < 0: # if threshold fell below queue length
                    Rates_in_Arr[i, j, k * upsample_factor + t] = -C
                    Queue_Length_dt[i, j] = Rates_in_Arr[i, j, t] * 1 / upsample_factor  # Qi(dt) = -C*dt
                    Queue_i_Length_Arr[i, j, k * upsample_factor + t] = torch.max(Queue_i_Length_Arr[
                                                                                      i, j, k * upsample_factor + t - 1] + \
                                                                                  Queue_Length_dt[i, j],
                                                                                  torch.tensor(0))
                else:  # trying to reach steady state
        ...
        for i in range(N_ports):
            for j in range(N_streams[i]):
                if Delta_Arr[i, j, k * upsample_factor + t] < 0:
                    if Delta_Arr[i, j, k * upsample_factor + t - 1] > 0:  # only if in transient state:
                        if j == 0:  # for high priority queue
                            ...
                            Intr = (Queue_i_Length_Arr[i, j, k * upsample_factor + t - 1] * Threshold[
                                0, k * upsample_factor + t] - Threshold[0, k * upsample_factor + t - 1] *
                                    Queue_i_Length_Arr[i, j, k * upsample_factor + t]) \
                                   / (Queue_i_Length_Arr[i, j, k * upsample_factor + t - 1] + Threshold[
                                0, k * upsample_factor + t] - Threshold[
                                          0, k * upsample_factor + t - 1] - Queue_i_Length_Arr[
                                          i, j, k * upsample_factor + t])

                            Corr_q_i = Queue_i_Length_Arr[i, j, k * upsample_factor + t] - Intr
                            Corr_Thr = Intr - Threshold[0, k * upsample_factor + t]
                            Corr_h = Corr_Thr - Corr_q_i  # the correction needed to the threshold
                        else:  # for low priority queue
                            # calculate the intersection point between Low Priority Threshold and a specific Queue length
                            Intr = (Queue_i_Length_Arr[i, j, k * upsample_factor + t - 1] * Threshold[
                                1, k * upsample_factor + t] - Threshold[1, k * upsample_factor + t - 1] *
                                    Queue_i_Length_Arr[i, j, k * upsample_factor + t]) \
                                   / (Queue_i_Length_Arr[i, j, k * upsample_factor + t - 1] + Threshold[
                                1, k * upsample_factor + t] - Threshold[
                                          1, k * upsample_factor + t - 1] - Queue_i_Length_Arr[
                                          i, j, k * upsample_factor + t])
                            Corr_q_i = Queue_i_Length_Arr[i, j, k * upsample_factor + t] - Intr
                            Corr_Thr = Intr - Threshold[1, k * upsample_factor + t]
                            Corr_l = Corr_Thr - Corr_q_i  # the correction needed to the threshold
                        ...
                        Rates_in_Arr[i, j, k * upsample_factor + t] = Delta_Arr[i, j, k * upsample_factor + t] / (
                                1 / upsample_factor)
                        if Rates_in_Arr[i, j, k * upsample_factor + t] < -C:
                            Rates_in_Arr[i, j, k * upsample_factor + t] = -C
                        Queue_Length_dt[i, j] = Rates_in_Arr[i, j, k * upsample_factor + t] * 1 / upsample_factor
                        #####################################################
                        # Lost_Packets_dt_Arr[i, j, k * upsample_factor + t] = -Delta_Arr[
                        #     i, j, k * upsample_factor + t]
                        ########################################
        # re-calculate Q(t) and the Thresholds (needed only if there was any correction done):
        if not (k == 0 and t == 0):
            Q[k * upsample_factor + t] = Q[k * upsample_factor + t - 1] + Queue_Length_dt.sum((0, 1))
            Threshold[0, k * upsample_factor + t] = torch.min(alpha_high * (B - Q[k * upsample_factor + t] + Corr_h),
                                                              torch.tensor(alpha_high * B))
            Threshold[1, k * upsample_factor + t] = torch.min(alpha_low * (B - Q[k * upsample_factor + t] + Corr_l),
                                                              torch.tensor(alpha_low * B))
            ###################################
            # add CORRECTION FACTOR ROBUST TO ALL QUEUES
            #################################
        ...
        for i in range(N_ports):
            for j in range(N_streams[i]):
                if Delta_Arr[i, j, k * upsample_factor + t] < 0:
                    if Delta_Arr[i, j, t - 1] != 0:  # only if overshoot / wasn't in steady state already:
                        if j == 0:  # for high priority queue
                        ...
                        Delta_Arr[i, j, k * upsample_factor + t] = 0
                    else:  # if it reached steady state (for one of the queues)
                        Queue_i_Length_Arr[i, j, k * upsample_factor + t] = Queue_i_Length_Arr[
                                                                                i, j, k * upsample_factor + t - 1] + \
                                                   Queue_Length_dt[i, j]
                        if j == 0:
                            # re-calculate high delta:
                            Delta_Arr[i, j, k * upsample_factor + t] = Threshold[0, k * upsample_factor + t] - \
                                                                       Queue_i_Length_Arr[i, j, k * upsample_factor + t]
                        else:
