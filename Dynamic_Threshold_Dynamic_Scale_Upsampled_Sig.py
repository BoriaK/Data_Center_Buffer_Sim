...
upsample_factor = 11
Upsampled_Traffic = torch.from_numpy(UpsamleZOH(Traffic, upsample_factor))
Scaled_Upsampled_Traffic = torch.zeros(Length * upsample_factor)
...
for i in range(Length * upsample_factor):
    ...
    Diff = Threshold[i] - Scaled_Upsampled_Traffic[i]
    # if Diff < 0:
    #     New_Scale = 0
    # else:
...
Time = range(0, len(Upsampled_Traffic))
plt.figure()
plt.plot(Time, Scaled_Upsampled_Traffic, Time, Threshold)
plt.legend(['q_1', 'T_1'])
plt.grid()
plt.show()
# print(Threshold)
